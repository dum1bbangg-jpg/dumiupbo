...
사이트가 위플랩에 직접 접속할 수는 없습니다. 위플랩 로그인이 룰렛업보정리기 안에만
있기 때문입니다. 그래서 지난 기록은 이렇게 넣습니다.

1. 룰렛업보정리기에서 엑셀로 내보내기
2. `아이디 · 닉네임 · 업보내용 · 분류 · 상태 · 날짜` 순으로 열 정리
3. 관리자 → **한 번에 등록** 에 붙여넣기 → **미리 검사** → **전부 등록**
